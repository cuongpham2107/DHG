<div class="col-md-12">
  <div class="table-responsive">
      <table class="table box table-bordered">
          <thead>
              <tr style="background: #cad9e9">
                  <th style="width: 50px;">No.</th>
                  <th>{{ sc_language_render('product.name') }}</th>
                  <th>{{ sc_language_render('product.price') }}</th>
                  <th  style="width: 90px;">{{ sc_language_render('product.quantity') }}</th>
              ...
              <tr class="row_cart form-group {{ session('arrErrorQty')[$product->id] ?? '' }}{{ (session('arrErrorQty')[$product->id] ?? 0) ? ' has-error' : '' }}">
                  ...
                  <td>
                      <a href="{{$product->getUrl() }}" class="row_cart-name">
                          ...
                          <span>
                            <a href="{{$product->getUrl() }}" class="row_cart-name" style="color:#020202">{{ $product->name }}</a><br />
                  ...
                  <td>
                    @if($product->promotionPrice != null)
                        {{ number_format($product->promotionPrice->price_promotion) }}
                    @else
